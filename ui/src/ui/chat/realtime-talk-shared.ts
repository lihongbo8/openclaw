...
function getTerminalAgentWaitError(result: AgentWaitResult | undefined): Error | undefined {
  ...
  if (result.status === "error") {
    return new Error(message || "本机系统工具调用失败");
  }
  ...
  if (hasTerminalTimeoutMetadata) {
    return new Error(message || "本机系统工具调用超时");
  }
  return undefined;
}
...
function waitForChatResult(params: {
  ...
  return new Promise((resolve, reject) => {
    if (params.signal?.aborted) {
      reject(new DOMException("本机系统工具调用已取消", "AbortError"));
      return;
    }
    const timer = window.setTimeout(() => {
      settleReject(new Error("本机系统工具调用超时"));
    }, params.timeoutMs);
    let settled = false;
    let emptyFinalWaitStarted = false;
    let emptyFinalFallbackTimer: number | undefined;
    const onAbort = () => {
      settleReject(new DOMException("本机系统工具调用已取消", "AbortError"));
    };
    ...
    unsubscribe = params.client.addEventListener((evt: GatewayEventFrame) => {
      ...
      if (payload.state === "final") {
        ...
        settleReject(
          new DOMException(payload.errorMessage ?? "本机系统工具调用已取消", "AbortError"),
        );
      } else if (payload.state === "error") {
        settleReject(new Error(payload.errorMessage ?? "本机系统工具调用失败"));
      }
    });
    function cleanup() {
...
export async function submitRealtimeTalkConsult(params: {
  ...
  try {
    ...
    if (!runId) {
      throw new Error("本机系统实时工具调用没有返回运行编号");
    }
    if (params.signal?.aborted) {
      abortRun();
      submitAbortResult();
